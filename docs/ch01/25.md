...
$\Rightarrow$ 方向：$f_{(1)}(x)\le\dots\le f_{(k)}(x)\le a$，已是 $k$ 个不大于 $a$ 的值。$\Leftarrow$ 方向：若有 $k$ 个值 $\le a$，则从小到大排列后第 $k$ 个必 $\le a$，即 $f_{(k)}(x)\le a$。

而至少有 $k$ 个下标 $i$ 使 $f_i(x)\le a$ 这一条件可写成
